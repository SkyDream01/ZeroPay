    ...
    @Test public void createBundleSelectRestoreAndCheckout(){
        ...
        try(StoreDb db=new StoreDb(context)){
            ...
            try(var controller=Robolectric.buildActivity(MainActivity.class).setup()){
                MainActivity a=controller.get();find(a.getWindow().getDecorView(),"库存").performClick();
                find(a.getWindow().getDecorView(),"套餐管理").performClick();
                find(ShadowAlertDialog.getLatestAlertDialog().getWindow().getDecorView(),"+ 新建套餐").performClick();
                ...
                ListView products=list(ShadowAlertDialog.getLatestAlertDialog().getWindow().getDecorView());products.performItemClick(null,0,0);
                ...
                find(a.getWindow().getDecorView(),"收银").performClick();find(a.getWindow().getDecorView(),"选择套餐").performClick();
                find(ShadowAlertDialog.getLatestAlertDialog().getWindow().getDecorView(),"加入套餐").performClick();
                ...
                org.robolectric.Shadows.shadowOf(android.os.Looper.getMainLooper()).idle();ShadowAlertDialog.getLatestAlertDialog().getButton(AlertDialog.BUTTON_POSITIVE).performClick();
    ...
    private void checkPromotionCheckout(int type,String description,long total){
        ...
        try(StoreDb db=new StoreDb(context)){
            ...
            try(var controller=Robolectric.buildActivity(MainActivity.class).setup()){
                ...
                AlertDialog rule=ShadowAlertDialog.getLatestAlertDialog();spinner(rule.getWindow().getDecorView()).setSelection(type);
